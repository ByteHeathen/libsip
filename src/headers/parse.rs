...
macro_rules! impl_array_parser {
    ($name:tt, $tag:tt, $variant:ident) => {
        named!(pub $name<Header>, do_parse!(
            ...
            char!(':') >>
            opt!(take_while!(is_space)) >>
            data: separated_list!(char!(','), parse_method) >>
            (Header::$variant(data))
        ));
    }
}
...
impl_string_parser!(parse_supported_header, "Supported", Supported);
...
impl_array_parser!(parse_accept_header, "Accept", Accept);
impl_array_parser!(parse_allow_header, "Allow", Allow);
